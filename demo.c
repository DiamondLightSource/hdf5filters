...
to discover, load and register filters at runtime.
...
#define FILE "demo-lz4.h5"
#define DATASET "DS1"
#define DIM0 32
#define DIM1 64
#define CHUNK0 4
#define CHUNK1 8
#define H5Z_FILTER_LZ 32004
 ...
 */
    avail = H5Zfilter_avail(H5Z_FILTER_LZ);
    if (avail) {
        status = H5Zget_filter_info(H5Z_FILTER_LZ, &filter_config);
        if ((filter_config & H5Z_FILTER_CONFIG_ENCODE_ENABLED) &&
            (filter_config & H5Z_FILTER_CONFIG_DECODE_ENABLED))
            printf("LZ4 filter is available for encoding and decoding.\n");
    }
    else {
        printf("LZ4 filter is not available. Check your filter library and environment. Aborting\n");
        status = H5close();
        return -1;
    }
 ...
 * Example: Writing and Reading Compressed Data
 * Page 19 of 26
*/
 ...
 */
    ...
    status = H5Pset_filter(dcpl, H5Z_FILTER_LZ, H5Z_FLAG_MANDATORY,
                           (size_t) 6, cd_values);
 ...
 * for bzip2.
 */
    filter_id = H5Pget_filter2(dcpl, (unsigned) 0, &flags, &nelmts,
 ...
 */
    avail = H5Zfilter_avail(H5Z_FILTER_LZ);
    if (!avail)
        printf("Filter is not yet available after the H5Pget_filter call.\n");
    else
        return 1;
